 ...
 * 请求经 /api 前缀由 src/mock/renewal.ts 拦截，无需真实后端。
 */
...
export function getRenewalList(params: RenewalQuery & PageParams) {
  return get<PageResult<Renewal>>('/renewal/list', params as Record<string, any>)
}

/** 保源详情 */
export function getRenewalDetail(id: number) {
  return get<Renewal>('/renewal', { id } as Record<string, any>)
}

/** 新增保源 */
export function createRenewal(data: RenewalForm) {
  return post<Renewal>('/renewal', data as Record<string, any>)
}

/** 编辑保源（表单自带 id） */
export function updateRenewal(data: RenewalForm) {
  return put<Renewal>('/renewal', data as Record<string, any>)
}

/** 删除保源 */
export function deleteRenewal(id: number) {
  return del<void>('/renewal', { id } as Record<string, any>)
}
