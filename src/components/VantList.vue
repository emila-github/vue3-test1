...
const sheetActions = computed(() => {
  ...
  const custom = props.actions
    ...
    .map((a) => ({ name: a.name, value: `act:${a.key}`, icon: a.icon || 'ellipsis', danger: a.danger }))
  return [...base, ...custom]
})
  ...
  <div class="vant-list picc-page" :data-perm="permTick">
    ...
    <van-popup v-model:show="formVisible" position="right" :style="{ width: '100%', height: '100%' }">
      <van-nav-bar :title="isEdit ? '编辑' : '新增'" left-arrow @click-left="formVisible = false">
        <template #right>
          <van-button type="primary" size="small" :loading="submitting" @click="submit">提交</van-button>
        </template>
      </van-nav-bar>
