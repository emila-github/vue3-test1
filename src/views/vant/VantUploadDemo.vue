...
<template>
  <div class="upload-demo">
    ...
    <div class="card">
      ...
      <VantUpload v-model="avatar" type="avatar" :upload="mockUpload" @change="onChange('头像', $event)" />
    </div>
    ...
    <div class="card">
      ...
      <p class="hint">· <code>type="avatar"</code> 圆形头像；<code>type="idcard"</code> 配合 <code>variant="front|back"</code> 展示身份证正反面占位。</p>
      <p class="hint">· <code>type="document"</code> 证件上传，自定义 UI 展示图标 / 名称 / 大小 / 进度 / 删除，支持 PDF。</p>
      <p class="hint">· <code>type="image"</code> + <code>multiple</code> 支持多图片上传；多选时 <code>v-model</code> 为 string[]。</p>
      <p class="hint">· 传入 <code>:upload="fn"</code> 自定义真实上传；不传则使用本地 ObjectURL 预览。</p>
      <p class="hint">· 支持 <code>label</code> / <code>required</code> / <code>disabled</code> / <code>readonly</code> / <code>maxSize</code> / <code>accept</code>。</p>
    </div>
  </div>
</template>
