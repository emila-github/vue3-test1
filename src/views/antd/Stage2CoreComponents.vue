...
const columns: TableColumnsType = [
  ...
  { title: '地址', dataIndex: 'address', key: 'address' },
  {
    ...
    customRender: ({ text }: { text: string[] }) =>
      text?.map((t) =>
        h('a-tag', { key: t, color: t === 'vue' ? '#52c41a' : t === 'react' ? '#1677ff' : 'default' }, () => t),
      ),
  },
]
...
<template>
  <div class="stage-page">
    ...
    <section class="card">
      <h2>2.1 Form 表单校验</h2>
      <p>
        ⚠️ antdv 4.x 已移除 <code>setFieldsValue</code>，直接修改 <code>model</code> 对象即可实现回显。
      </p>
      <a-form
        ref="formRef"
        :model="formState"
    ...
    <section class="card">
      ...
      <p>
        ⚠️ 4.x 中使用 <code>open</code> 而非 <code>visible</code>；二次封装务必加 <code>destroyOnClose</code>。
      </p>
      <a-space>
