...
const { data: cacheData, loading: cacheLoading } = useRequest<{
  version: string
  random: number
}>(
  () => get('/vr/demo/status'),
...
const { run: runRetry, loading: retryLoading } = useRequest<{ result: string }>(
  () => get('/vr/demo/retry'),
  {
    manual: true,
    errorRetryCount: 3,
...
const { data: focusData } = useRequest<{ version: string; random: string }>(
  () => get('/vr/demo/status'),
  {
...
// ========== 5. mutate 乐观更新 ==========
const { data: mutateData, mutate } = useRequest<{ version: string; random: string }>(
  () => get('/vr/demo/status'),
)
  ...
  <div class="advanced-demo">
    ...
    <a-card title="缓存 + SWR" size="small" style="margin-bottom:16px">
      ...
      <a-space style="margin-top:8px">
        <a-button size="small" @click="clearCache()">clearCache()</a-button>
      </a-space>
      <div class="code-hint">
        10 秒内多次进入，直接使用缓存不发起请求；10 秒后先返回缓存再后台更新。
      </div>
    </a-card>
    ...
    <a-card title="轮询 (pollingInterval)" size="small" style="margin-bottom:16px">
      ...
      <a-descriptions size="small" :column="2" style="margin-top:8px">
        ...
        <a-descriptions-item label="时间戳">{{ pollData?.timestamp ? new Date(pollData.timestamp).toLocaleTimeString() : '-' }}</a-descriptions-item>
      </a-descriptions>
      <div class="code-hint">
        每 2 秒自动轮询，数据实时更新。关闭开关 → <code>ready=false</code> 自动停止。
      </div>
    </a-card>
    ...
    <a-card title="错误重试 (50% 失败率)" size="small" style="margin-bottom:16px">
      <a-space>
        <a-button type="primary" :loading="retryLoading" @click="runRetry()">
          发起请求（自动重试 3 次）
        </a-button>
        <a-tag>{{ retryLogs.length ? (retryLogs[retryLogs.length - 1]?.startsWith('✅') ? '成功' : '失败') : '-' }}</a-tag>
      </a-space>
      <a-list v-if="retryLogs.length" size="small" style="margin-top:8px">
        <a-list-item v-for="(log, i) in retryLogs" :key="i" :style="{ color: log.startsWith('✅') ? '#52c41a' : '#ff4d4f' }">
          {{ log }}
        </a-list-item>
      </a-list>
      <div class="code-hint">
    ...
    <a-card title="窗口聚焦刷新" size="small" style="margin-bottom:16px">
      ...
      <div class="code-hint">
        <code>refreshOnWindowFocus: true</code> — 切到其他 Tab 后再切回来，数据自动刷新。
      </div>
    </a-card>
    ...
    <a-card title="mutate — 直接修改 data" size="small">
      ...
      <a-button size="small" style="margin-top:8px" @click="handleMutate">
        mutate({ version: '99.9.9' })
      </a-button>
      <div class="code-hint">
